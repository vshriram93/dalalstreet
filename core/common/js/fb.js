...
function sendRequestViaMultiFriendSelector(accessToken,evtId) {
    FB.ui({
	...
	if (response && !response.error) {
	    console.log(response);
	    FB.api('/'+evtId+'/invited?users='+response.to,'post',function(resp) {
		console.log(event); 
	    });
    };
    }
...
function createFBEvent(name,description,start_time,end_time,location,callback) {
...
//    getEventInvitee(accessToken);
	...
	{
	    sendRequestViaMultiFriendSelector(accessToken,event.id);
	    callback(event.id);
